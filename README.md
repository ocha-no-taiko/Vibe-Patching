...
D2 からは、管理者画面で指定したBPMのクロックパルスを SYNC IN に出す（→「[SYNCクロック出力](#syncクロック出力)」）。
...
│   ├── sync_config.json      # SYNCクロックのON/OFFとBPM（管理者画面から変更）
...
- 現在のSYNCクロックのBPM（またはOFF）を表示（変更は管理者画面のみ）
...
| **Sync Clock** | SYNCクロックのBPM（20〜300）とON/OFFを設定。`sync_config.json` に保存され、再起動後も残る |
| **Direct CV** | CV1〜CV6 を 0〜255 の数値で直接指定し、キューを無視して即座に適用。**Load Current** で現在値を入力欄に読み込む |
| **Clear Queue** | キューを全消去 |
...
| `python/sync_config.json` | SYNCクロックのON/OFFとBPM（通常は管理者画面から変更） |
...
D2 から volca modular の SYNC IN にクロックパルスを出し、シーケンサーのテンポを管理者画面から指定する。
...
- BPMを変えると、次のステップから新しいテンポになる。
- 観客画面（Generate）には現在のBPMが表示される。変更できるのは管理者だけ。

### 設定（`python/sync_config.json`）

通常は管理者画面の **Sync Clock** から変更する（この JSON に保存される）。直接編集してもアプリ再起動なしで反映される。
...
- BPMは 20〜300。管理者画面では範囲外を受け付けず、JSONを直接書いた場合は範囲内に丸められる。
- Python が起動時・設定変更時・JSONの更新時（2秒ごとにチェック）にマイコンへ送る。マイコン側の準備が終わるまでは自動で再送する。
- 同じ内容でも30秒ごとに送り直すので、スケッチだけを書き直してマイコンが再起動しても、30秒以内に設定が戻る。
